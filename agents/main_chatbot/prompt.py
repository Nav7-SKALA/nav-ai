...
- 출력 시 프로젝트명 내에 괄호([])는 절대 사용하지 마세요
- 일반화 가능한 로드맵 구성

**출력 형식:**
{{
  "similar_analysis_text": "로드맵 제시 전 유사 구성원의 경력 증진 경로임을 설명하는 간단한 마크다운 형식의 안내 1-2문장 (예시: 해당 커리어 목표를 달성한 사내 전문가들은 아래와 같은 경로로 성장했습니다.)",
  "project": {{
    "project": [
      {{
        "period": "예: 1-3년차",
...
❗주의: 출력 JSON 구조에서 project, experience, certification은 **각각 정확히 1번씩만 등장해야 합니다.**
...
**출력 형식:**
{{
  "career_path_text": "사용자의 현재 경력과 목표, 회사의 방향성을 고려한 친근한 대화체 조언을 마크다운 형식으로 작성 (1-2 문장. 예: 현재 2년차인 당신은 AI와 클라우드 분야의 접점을 넓혀야 합니다...)",
  "career_path_roadmap": [
    {{
...
:반드시: 프로젝트명 내에 []는 모두 제거하고 실제 내용만 작성
**중요 조건:**
...
당신은 HR 전문가입니다. 입력된 사내 구성원 정보를 분석하여 사용자의 커리어 관련 질문에 조언을 해줄 수 있는 가상의 멘토를 생성하세요.
...
- 멘토 정보 중 experience, certification은 실제 구성원 정보만을 활용하여 작성
...
**사용자 질문:** {user_query}
**사용자 관심 분야:** {skill_set}, {domain}, {job}
