...
- role_model: 사용자가 원하는 타입의 전문가(특정 직무 전문가, 유사 경력자, 전환 경험자)를 매칭하여 1:1 가상 멘토링 대화 제공
- trend_path: 사용자의 질의에 따라 적절한 사내 강의 침 수강 순서 추천
...
**role_model 에이전트의 경우:**
- 사용자가 **어떤 전문가와 대화하고 싶어하는지** 분석하여 재생성
- **특정 직무 전문가**를 원하는 경우: 해당 직무/역할만 명시 (현재 경력 정보 제외)
  - 예시: "백엔드 개발자 전문가와 대화하고 싶습니다"
- **유사한 경력의 전문가**를 원하는 경우: 사용자의 현재 경력 정보 포함
  - 예시: "3년차 마케터 경력을 가진 전문가와 대화하고 싶습니다"
- **전환 경험자**를 원하는 경우: 현재 경력 + 목표 직무 모두 포함
  - 예시: "마케터에서 백엔드 개발자로 전환한 전문가와 대화하고 싶습니다"
- 도메인/산업 분야 정보는 해당하는 경우에만 포함

**다른 에이전트들의 경우:**
- 사용자의 현재 직무, 경력년차, 기술 스택, 도메인을 구체적으로 명시
- 이때 직무는 {role}, 기술 스택은 {skill_set} 내에 있는 정보를 기반으로 작성
- 도메인/산업 분야는 질의에 있는 경우, {domain} 참고하여 작성
- 목표하는 직무/분야와 현재 경력의 연관성 분석 포함
...
- **제공된 role, job 목록만을 활용**하여 role, job에 대한 내용 작성
- 개인 식별이 불가능하도록 프로젝트명은 활용한 핵심 기술 스택(skill_set)을 포함한 프로젝트명으로 재생성
...
- :반드시: experience나 certification이 없으면 해당 필드를 아예 JSON에 포함하지 마세요.
...
**role, job, key_sills는 반드시 제공된 role, job, skill_set 목록만을 활용**하되, 회사의 미래 방향성을 반영하여 작성
**회사의 미래 방향성과 핵심 기술 트렌드에 부합**하는 project, role, job 제시
...
- **회사의 핵심 방향성(예: AI 전환, 에이전틱 AI 등)**을 각 단계에 자연스럽게 통합
...
- :반드시: common_experience, common_cert의 경우, 사내 구성원의 실제 데이터를 출력. 만약 사용 가능한 정보가 없으면 임의로 생성하지 말고 해당 필드를 아예 JSON에 포함하지 마세요.
...
**작업 요구사항:**
사용자의 질문에서 언급된 **구체적인 직무/역할**에 대한 전문가 멘토를 생성하기 위해, 해당 직무에 **직접적으로 관련된** 다양한 정보를 종합적으로 수집해야 합니다.

**정보 수집 범위:**
1. **기술적 전문성**: 필요한 기술 스택, 도구, 방법론
2. **실무 경험**: 실제 프로젝트 사례, 업무 경험담
3. **업계 인사이트**: 현재 트렌드, 미래 전망, 시장 동향
4. **성장 경로**: 경력 개발 방법, 학습 리소스, 자격증
5. **실무자 관점**: 현직자의 조언, 경험담, 팁

**핵심 규칙:**
1. 사용자가 명시한 직무명을 반드시 키워드에 포함
2. 해당 직무의 다양한 측면을 다각도로 탐색
3. 관련 없는 포괄적인 직무는 제외
4. 실제 현업에서 활용 가능한 구체적인 정보 우선

**키워드 생성 가이드:**
- 영어로 작성 (글로벌 검색 결과 확보)
- 8-12단어 이내로 구성
- 사용자가 언급한 정확한 직무명 포함 필수
- 다양한 각도의 정보 수집을 위한 키워드 조합:
  * 기술 스택 및 도구
  * 실무 경험 및 프로젝트 사례
  * 업계 동향 및 트렌드
  * 자격증 및 교육 과정
...
**예시:**
사용자 질문: "백엔드 개발자로 성장하고 싶어"
✅ 다양한 각도의 키워드:
- "backend developer technical skills experience"
- "backend engineer project case studies"
- "backend development industry trends 2024"
- "backend developer certification requirements"
- "senior backend engineer career experience"
...
**사용자 질문:** {user_query}
**활용 가능한 사내 정보:** 기술 스택: {skill_set}, 도메인: {domain}, 수행 역할: {job}

**외부 전문가 검색 결과:**
{external_info}

**핵심 제약사항:**
- 사용자가 질문한 **구체적인 직무/역할**에만 집중
- 검색 결과에서 해당 직무와 직접 관련된 정보만 활용
- 관련 없는 포괄적인 직무 정보는 무시
- 멘토는 사용자가 언급한 그 직무의 전문가여야 함

**작업 요구사항:**
- 그룹명: "외부 전문가"로 고정
- 검색된 다양한 정보를 **종합적으로 분석**하여 현실적인 가상 멘토 생성
- 사용자가 언급한 직무와 직접 관련된 내용만 활용
- 모든 응답은 한국어로 작성
- 멘토 이름은 한국어로 자연스럽게
- **현실적인 전문가 프로필**: 실제 업계에 존재할 법한 경험과 전문성 반영
- 다양한 정보를 조합하여 **일관성 있는 인물** 생성
- member_count는 검색 결과 기반으로 설정 (보통 3-5명)
- real_info는 빈 리스트로 설정
- **중요**: common_experience, common_cert 등은 검색된 **실제 데이터를 종합**하여 현실적으로 구성

**멘토 생성 철학:**
- 검색된 정보들을 **스토리텔링**하여 하나의 일관된 인물로 구성
- 기술적 전문성 + 실무 경험 + 업계 인사이트를 균형있게 반영
- 단순한 정보 나열이 아닌 **경험에 기반한 조언**을 제공하는 인물
- 해당 직무의 **현실적인 어려움과 기회**를 모두 아는 전문가

**직무 일치성 검증:**
생성하기 전에 다음을 확인하세요:
1. 멘토의 current_position이 사용자 질문의 직무와 일치하는가?
2. main_domains가 해당 직무 영역에 속하는가?
3. common_skill_set이 그 직무에 특화된 기술인가?
4. 일반적/포괄적인 내용이 아닌 구체적인 직무 전문 내용인가?
...
{{
  "group_name": "외부 전문가",
  "group_description": "해당 직무의 구체적인 전문 영역 설명",
  "common_skill_set": ["해당 직무 전용 기술1", "해당 직무 전용 기술2"],
  "common_career_path": ["해당 직무 커리어 단계1", "해당 직무 커리어 단계2"],
  "role_model": {{
    "name": "홍길동",
    "current_position": "사용자 질문의 구체적인 직무명",
    "experience_years": "10년",
    "main_domains": ["해당 직무 전문 도메인1", "해당 직무 전문 도메인2"],
    "advice_message": "해당 직무에 특화된 조언 메시지"
  }},
  "real_info": [],
  "common_project": ["해당 직무 관련 프로젝트1", "해당 직무 관련 프로젝트2"],
  "common_experience": ["해당 직무 관련 경험1", "해당 직무 관련 경험2"],
  "common_cert": ["해당 직무 관련 자격증1", "해당 직무 관련 자격증2"]
}}
주의사항:

JSON 외의 출력은 허용되지 않습니다.
사용자 질문에서 명시된 직무와 정확히 일치하는 전문가만 생성하세요.
포괄적이거나 관련성이 낮은 정보는 제외하세요.
"""
